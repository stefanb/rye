		...
		case env.Block:
			...
			if obj1.Mode == 1 {
				...
				if len(keys) > 0 {
					...
					return _emptyRM(), evaldo.MakeBuiltinError(ps, "No selectors before tag map.", "process")
				}
			}
		default:
			// ni Dict ampak blok kode, vrni blok
			return _emptyRM(), evaldo.MakeBuiltinError(ps, "Unknow type in block parsing TODO.", "process")
		}
	}
	return rmap, nil
}
	...
	for key, val := range data.Data {
		...
		if ok0 {
			...
			case env.Block:
				...
				ps.Ser = obj.Series
				evaldo.EvalBlockInj(ps, env.ToRyeValue(val), true)
				if ps.ErrorFlag {
					ps.Ser = ser
					return ps.Res
				}
...
func dictToStruct(ps *env.ProgramState, dict env.Dict, structPtr any) env.Object {
	...
	if structVal.Kind() != reflect.Ptr {
		return evaldo.MakeBuiltinError(ps, "Second argument must be a pointer to a struct", "dict->struct")
	}
	...
	for i := 0; i < structVal.NumField(); i++ {
		...
		if err := setFieldValue(field, dictValue); err != nil {
			return evaldo.MakeBuiltinError(ps, fmt.Sprintf("Error setting field %s: %s", fieldName, err.Error()), "dict->struct")
		}
	}
...
func structToDict(ps *env.ProgramState, structPtr any) env.Object {
	...
	// Check if it's a struct
	if structVal.Kind() != reflect.Struct {
		return evaldo.MakeBuiltinError(ps, "Argument must be a struct or pointer to a struct", "struct->dict")
	}
