...
var Builtins_http = map[string]*env.Builtin{
	...
	/* "Go-server//serve\\port": {
		...
		Fn: func(ps *env.ProgramState, arg0 env.Object, arg1 env.Object, arg2 env.Object, arg3 env.Object, arg4 env.Object) env.Object {
			...
			default:
				ps.FailureFlag = true
				return evaldo.MakeArgError(ps, 1, []env.Type{env.StringType}, "Go-server//serve\\port")
			}
				...
				case env.Function:
					...
					http.HandleFunc(path.Value, func(w http.ResponseWriter, r *http.Request) {
						...
						if err != nil {
							...
							// TODO return evaldo.MakeError(ps, err.Error())
						}
				...
				case env.String:
					...
					case env.Function:
						http.HandleFunc(path.Value, func(w http.ResponseWriter, r *http.Request) {
							...
							// fmt.Println(ctx)
							fmt.Println("<< // Call Function Args 2 >>")
							evaldo.CallFunctionArgs2(handler, env1, *env.NewNative(env1.Idx, c, "Go-server-websocket"), *env.NewNative(env1.Idx, r.Context(), "Go-server-context"), nil)
						})
						return arg0
					default:
						env1.FailureFlag = true
